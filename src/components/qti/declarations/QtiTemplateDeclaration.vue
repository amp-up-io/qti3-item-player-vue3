...
export default {
  ...
  methods: {
    ...
    /**
     * @description Basic validation of the only possible child:
     * qti-default-value (0-1)
     */
    validateChildren () {
      ...
      this.$slots.default().forEach((slot) => {
        if (qtiAttributeValidation.isValidSlot(slot)) {
          ...
          throw new QtiValidationException('Element must be qti-default-value: "' + slot.type.name + '"')
        }
      })
    },
