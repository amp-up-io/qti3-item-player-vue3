...
export default class QtiAttributeValidation {
  ...
  /**
   * @description Convert a PascalCase string to kebab-case string
   * @param {*} str 
   ...
   */
  isValidSlot (slot) {
    if (typeof slot.type.name !== 'undefined') return true

    // Check if Symbol slot, and is something not empty
    if ((typeof slot.type === 'symbol') && (slot.children.trim().length > 0)) {
      
      // Comment slot.  Skip it.
      if (slot.type.toString() === 'Symbol(v-cmt)') return false

      // Not an empty text slot.  This is an error.
      throw new QtiValidationException('Invalid Child Node: "' + slot.children.trim() + '"')
    }
