  ...
  <div 
    ...
    <div class="inline-choice-wrapper">
      ...
      <ul ref="listbox"
        ...
        <qti-inline-choice 
          identifier="" 
          fixed="true">{{dataPrompt}}</qti-inline-choice>
        <slot></slot>
      </ul>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    validateChildren () {
      ...
      this.$slots.default().forEach((slot) => {
        if (qtiAttributeValidation.isValidSlot(slot)) {
          if (slot.type.name === 'QtiInlineChoice') return
          throw new QtiValidationException('Node is not a qti-inline-choice: "' + slot.type.name + '"')
        }
      })
    },
     ...
     */
    processChildren () {
      // First child (children[0]) is the "Choose..." node
      const chooseNode = this.$.subTree.children[0].children[2].children[0]
      this.choices.push(chooseNode.component.proxy)

      // Add the remaining choices
      const children = this.$.subTree.children[0].children[2].children[1].children
    ...
    addChoiceMousedownListeners () {
      this.allOptionElements = this.$refs.listbox.querySelectorAll('[role="option"]')
      this.allOptionElements.forEach((option) => {
        option.addEventListener('mousedown', this.handleChoiceMousedown)
      })
    },

    removeChoiceMousedownListeners () {
      if (this.allOptionsElements === null) return
      this.allOptionElements.forEach((option) => {
        option.removeEventListener('mousedown', this.handleChoiceMousedown)
      })
    }
  ...
  mounted () {
    if (this.isQtiValid) {
      try {
        ...
        // Bind handler for a mousedown event coming from each Choice in the listbox
        this.addChoiceMousedownListeners()
  ...
  beforeUnmount () {
    this.removeChoiceMousedownListeners()
  }
}
</script>
...
.inline-choice-select-label.qti-hidden {
  display: none;
}
