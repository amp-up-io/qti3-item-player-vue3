...
export default {
  ...
  methods: {
    ...
    validateChildren () {
      ...
      let associableHotspotCount = 0
      let foundImage = false

      this.$slots.default()[0].children.forEach((slot) => {
        if (qtiAttributeValidation.isValidSlot(slot)) {
          // Only qti-prompt, qti-gap-text, qti-gap-img, and qti-associable-hotspot's permitted
          if (slot.type.name === 'QtiPrompt') {
            if ((gapChoiceCount > 0) || (associableHotspotCount > 0) || (foundImage)) {
              throw new QtiValidationException('Invalid element order. qti-prompt must come before the background image, qti-gap-text\'s, qti-gap-img\'s, or qti-associable-hotspot\'s')
            }
          ...
          if ((slot.type.name === 'QtiGapText') || (slot.type.name === 'QtiGapImg')) {
            if (associableHotspotCount > 0) {
              throw new QtiValidationException('Invalid element order. qti-gap-text\'s and qti-gap-img\'s must come before qti-associable-hotspot\'s')
            }
            return gapChoiceCount += 1
          }

          if (slot.type.name === 'QtiAssociableHotspot') {
            return associableHotspotCount += 1
          }

          throw new QtiValidationException('Node is not one of qti-gap-text, qti-gap-img, or qti-associable-hotspot: "' + slot.type.name + '"')
        
        } else {
            // Search for an img, object, or picture element
            if ((slot.type === 'img') || (slot.type === 'object') || (slot.type === 'picture')) {
              // image must come after prompt, but before qti-gap-text, qti-gap-img, and hotspot
              if ((gapChoiceCount > 0) || (associableHotspotCount > 0)) {
                throw new QtiValidationException('Invalid element order. Background image must preceed qti-gap-text\'s, qti-gap-img\'s and qti-associable-hotspot\'s')
              }
              foundImage = true
            }
        }
    ...
    processChildren () {
      ...
      children.forEach((node) => {
        // Get a handle on the background image
        if ((node.type === 'img') || (node.type === 'object') || (node.type === 'picture')) {
          this.image = node.el
          return
        }
        ...
        } else if (node.type.name === 'QtiAssociableHotspot') {
          this.gaps.push(node.component.proxy)
        }
      })
    },
...
.qti-ggm-group-wrapper .gap-choice-text.draggable.dragging,
.qti-ggm-group-wrapper .gap-choice-img.draggable.dragging {
  position: fixed;
  left: 0;
  top: 0;
