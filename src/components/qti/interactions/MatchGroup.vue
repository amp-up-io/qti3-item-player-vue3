...
export default {
  ...
  methods: {
      ...
      this.$slots.default()[0].children.forEach((slot) => {
        if (qtiAttributeValidation.isValidSlot(slot)) {
          ...
          if (slot.type.name === 'QtiPrompt') {
            if (matchSetCount > 0)
              throw new QtiValidationException('Invalid element order. qti-prompt must come before qti-simple-match-set\'s')
          ...
          if (slot.type.name === 'QtiSimpleMatchSet') {
            return matchSetCount += 1
          }

          throw new QtiValidationException('Node is not a qti-simple-match-set: "' + slot.type.name + '"')
        }
      })
    ...
    processChildren () {
      ...
      children.forEach((node) => {
        if ((node.type.name === 'QtiSimpleMatchSet') && (node.component !== null)) {
          const matchset = node.component.proxy
          // Stamp shuffle onto the matchset node
          matchset.setIsShuffle(this.isShuffle)
  ...
  mounted () {
    if (this.isQtiValid) {
      try {
        // Process the qti-simple-match-sets
