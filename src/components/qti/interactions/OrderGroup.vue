...
export default {
  ...
  methods: {
    ...
    setPromptElement (promptElement) {
    ...
    validateChildren () {
      ...
      this.$slots.default()[0].children.forEach((slot) => {
        if (qtiAttributeValidation.isValidSlot(slot)) {
          // Only qti-prompt, and qti-simple-choice's permitted
          if (slot.type.name === 'QtiPrompt') {
            if (choiceCount > 0) {
              throw new QtiValidationException('Invalid element order. qti-prompt must come before qti-simple-choice\'s')
            }
  ...
  mounted () {
    if (this.isQtiValid) {
      try {
        ...
        // Validate and process the choices.
